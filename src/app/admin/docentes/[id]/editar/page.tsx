...
export default function Page() {
  ...
  const [nombre, setNombre] = React.useState('Dr. García');
  const [foto, setFoto] = React.useState<string | null>(null);
  const [curso1, setCurso1] = React.useState('Matemática Preuniversitaria');
  const [curso2, setCurso2] = React.useState('Álgebra Preuniversitaria');
  ...
  return (<form onSubmit={handleSubmit} className="flex flex-col gap-6">
    <div>
      <Breadcrumb items={[
        ...
        { label: nombre || 'Docente', href: '/admin/docentes/1' },
        { label: 'Editar' }
      ]} />
    ...
    <Card title="Datos personales y contacto">
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
        ...
        <div className="lg:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
          <Input label="Código" defaultValue="DOC-001" disabled />
          <Input 
            ...
            required 
          />
          <Input label="DNI" defaultValue="10293847" placeholder="Ej: 10293847" hint="Debe tener exactamente 8 dígitos." required />
          <Input label="Especialidad" defaultValue="Matemática" placeholder="Ej: Matemática, Física, Química" required />
          <Input label="Correo Institucional" defaultValue="garcia@academia.com" placeholder="Ej: cgarcia@academia.com" required />
          <Input label="Teléfono" defaultValue="+51 987 654 321" placeholder="Ej: +51 987 654 321" required />
          <Input label="Dirección" defaultValue="Av. Los Pinos 123, Trujillo" placeholder="Ej: Av. Los Pinos 123, Trujillo" className="md:col-span-2" />
        </div>
      </div>
    </Card>
